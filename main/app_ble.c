...
extern uint8_t ESP_WIFI_SSID[WIFI_SSID_MAX_LEN];
extern uint8_t ESP_WIFI_PASS[WIFI_PASS_MAX_LEN];
extern char device_id[DEVICE_ID_MAX_LEN];
extern char client_cert[CLIENT_CERT_MAX_LEN];
extern char client_key[CLIENT_KEY_MAX_LEN];
extern char root_cert[ROOT_CERT_MAX_LEN];

static uint8_t ble_config_flag = 0;
...
#define GATTS_DEVICE_ID_VAL_LEN_MAX 0x40
...
static esp_attr_value_t gatts_device_id_val =
{
    .attr_max_len = GATTS_DEVICE_ID_VAL_LEN_MAX,
    .attr_len     = sizeof(device_id),
    .attr_value   = (uint8_t *)device_id,
};
...
void parseConfig(char *str)
{
    ...
    if(cJSON_GetObjectItem(conf, "pass")){
        ...
        memcpy(ESP_WIFI_PASS, wifi_pass, WIFI_PASS_MAX_LEN);
    }
    if(cJSON_GetObjectItem(conf, "client_cert")){
        char *cert = cJSON_GetObjectItem(conf, "client_cert")->valuestring;
        ESP_LOGI(TAG, "CLIENT CERTIFICATION = %s", cert);
        memcpy(client_cert, cert, CLIENT_CERT_MAX_LEN);
    }
    if(cJSON_GetObjectItem(conf, "client_key")){
        char *key = cJSON_GetObjectItem(conf, "client_key")->valuestring;
        ESP_LOGI(TAG, "CLIENT KEY = %s", key);
        memcpy(client_key, key, CLIENT_KEY_MAX_LEN);
    }
    if(cJSON_GetObjectItem(conf, "device_id")){
        char *id = cJSON_GetObjectItem(conf, "device_id")->valuestring;
        ESP_LOGI(TAG, "DEVICE ID = %s", id);
        memcpy(device_id, id, DEVICE_ID_MAX_LEN);
    }
...
{
    esp_err_t wifi_connected = ESP_OK;
    ...
    memcpy(wifi_config.sta.ssid, ESP_WIFI_SSID, WIFI_SSID_MAX_LEN);
    memcpy(wifi_config.sta.password, ESP_WIFI_PASS, WIFI_PASS_MAX_LEN);
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ...
        wifi_connected = ESP_OK;

    } else if (bits & WIFI_FAIL_BIT) {
        ESP_LOGI(TAG, "Failed to connect to SSID:%s, password:%s",
                 ESP_WIFI_SSID, ESP_WIFI_PASS);
        wifi_connected = ESP_FAIL;
    } else {
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_REG_EVT:
        ESP_LOGI(TAG, "REGISTER_APP_EVT, status %"PRIu32" , app_id %"PRIu32" \n", param->reg.status, param->reg.app_id);
        gl_profile_tab[PROFILE_A_APP_ID].service_id.is_primary = true;
        gl_profile_tab[PROFILE_A_APP_ID].service_id.id.inst_id = 0x00;
        gl_profile_tab[PROFILE_A_APP_ID].service_id.id.uuid.len = ESP_UUID_LEN_16;
        gl_profile_tab[PROFILE_A_APP_ID].service_id.id.uuid.uuid.uuid16 = GATTS_SERVICE_UUID_TEST_A;

        esp_err_t set_dev_name_ret = esp_ble_gap_set_device_name(device_id);
        if (set_dev_name_ret){
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        for (int i = 0; i < strlen(device_id); i++){
            rsp.attr_value.value[i] = device_id[i];
        }
...
esp_err_t readConfigFromNVS()
{
    ...
    if (err != ESP_OK) {
        ...
        uint8_t ssid_length = strlen((const char*)ESP_WIFI_SSID);
        ...
        uint8_t pass_length = strlen((const char*)ESP_WIFI_PASS);
...
esp_err_t setConfigFromNVS()
{
    ...
    if (err != ESP_OK) {
        ...
        printf("Set length of client certification from NVS ... ");
        uint8_t cert_length = strlen(client_cert);
        err = nvs_set_u8(my_handle, "client_cert_len", cert_length);
        if (err != ESP_OK) return err;
        else{
            printf("Done\n");
        }

        printf("Set client certification from NVS ... ");
        err = nvs_set_str(my_handle, "client_cert", (char*)client_cert);
        if (err != ESP_OK) return err;
        else {
            printf("Done\n");
        ...
        printf("Set length of client key from NVS ... ");
        uint8_t key_len = strlen(client_key);
        err = nvs_set_u8(my_handle, "client_key_len", key_len);
        if (err != ESP_OK) return err;
        else{
            printf("Done\n");
        }

        printf("Set client key from NVS ... ");
        err = nvs_set_str(my_handle, "client_key", (char*)client_key);
        if (err != ESP_OK) return err;
        else {
            printf("Done\n");
            printf("CLIENT KEY = %s\n", client_key);
        ...
        printf("Set length of device id from NVS ... ");
        uint8_t id_len = strlen(device_id);
        err = nvs_set_u8(my_handle, "device_id_len", id_len);
        if (err != ESP_OK) return err;
        else{
            printf("Done\n");
        }

        printf("Set device id from NVS ... ");
        err = nvs_set_str(my_handle, "device_id", (char*)device_id);
        if (err != ESP_OK) return err;
        else {
            printf("Done\n");
            printf("DEVICE ID = %s\n", device_id);
